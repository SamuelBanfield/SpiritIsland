        ...
        for land0 in lands0:
            for land1 in lands1:
                if land0 == land1:
                    ...
                    print("Explorers before: ", land0.invader_count["explorer"],
                          " | Explorers after: ", land1.invader_count["explorer"])
                    if land0.terrain in terrains: # Need to add check for invader source
                        assert land0.invader_count["explorer"] + 1 == land1.invader_count["explorer"], \
                            "Explorer not added in exploring terrain"
                    else:
                        assert land0.invader_count["explorer"] == land1.invader_count["explorer"], \
    ...
    def test_cards_advance(self):
        ...
        for round_number in range(len(invader_deck0)):
            ...
            if len(track_b["discard"]) > 0:
                assert track_a["ravage"] == track_b["discard"][-1], "Ravage card not moved into Discard"
            if track_b["ravage"] is not None:
                assert track_a["build"] == track_b["ravage"], "Build card not moved to Ravage"
    ...
    def test_build(self):
        ...
        for land0 in lands0:
            for land1 in lands1:
                if land0 == land1:
                    print("Check carried out in land: ", land0.number)
                    print("Explorers/Towns/Cities before: ",
                          land0.invader_count["explorer"], land0.invader_count["town"], land0.invader_count["city"],
                          " | Explorers/Towns/Cities after: ",
                          land1.invader_count["explorer"], land1.invader_count["town"], land1.invader_count["city"])
                    if land0.terrain in terrains:
                        if land0.invader_count["town"] > land0.invader_count["city"]:
                            assert land0.invader_count["city"] + 1 == land1.invader_count["city"], \
                                "City not built in building terrain"
                        else:
                            assert land0.invader_count["town"] + 1 == land1.invader_count["town"], \
                                "Town not built in building terrain"
                    else:
                        assert land0.invader_count["explorer"] == land1.invader_count["explorer"]\
                            and land0.invader_count["town"] == land1.invader_count["town"]\
                            and land0.invader_count["city"] == land1.invader_count["city"], \
    ...
    def test_ravage(self):  # Ravage will not cascade blight yet
        ...
        for land0 in lands0:
            for land1 in lands1:
                if land0 == land1:
                    print("Check carried out in land: ", land0.number)
                    print("Explorers/Towns/Cities/Dahan before: ",
                          land0.invader_count["explorer"], land0.invader_count["town"], land0.invader_count["city"], land0.dahan_count,
                          " | Explorers/Towns/Cities/Dahan after: ",
                          land1.invader_count["explorer"], land1.invader_count["town"], land1.invader_count["city"], land1.dahan_count)
                    if land0.terrain in terrains:
                        invader_damage = land0.invader_count["explorer"] + 2*land0.invader_count["town"] + 3*land0.invader_count["city"]
                        dahan_health = 2*land0.dahan_count
                        if invader_damage >= 2:
                            assert land0.blight_count + 1 == land1.blight_count, "Blight not added to blighted land"
                        assert dahan_health - invader_damage <= 2*land1.dahan_count, "Not enough dahan killed during ravage"
                        invader_health = invader_damage
                        dahan_damage = 2*land1.dahan_count
                        assert invader_health - dahan_damage <= land1.invader_count["explorer"] + 2*land1.invader_count["town"] + 3*land1.invader_count["city"], \
