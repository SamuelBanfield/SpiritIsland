        ...
        for index, stage in enumerate(["ravage", "build", "explore"]):
            ...
            if self._island.invader_track[stage]:
                ...
                rect = pygame.rect.Rect(
                    (self._margin + index * (self._card_width + self._margin), self._margin * 2 + self._text_height),
                    (self._card_width, self._card_height)
                )
                self._surface.blit(pygame.transform.scale(card, (self._card_width, self._card_height)), rect)
